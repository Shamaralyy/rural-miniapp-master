<template>
	<view class="editor-container">
		<!-- <view id="fixed-top" class="fixed-top" :class="{ isFixed: isFixed }"><button class="btn btn-primary" @click="save">保存</button></view>
	...
	methods: {
		...
		async chooseImage(onlyCamera) {
			...
			const success = res => {
				...
				this.imgList = [];
				uni.showLoading({ title: `正在上传 ${this.tempFilePaths.length} 张图片` });
				// 遍历文件列表  逐个接入风控检测
				this.tempFilePaths.map(async file => {
					const isSafe = await this.safe_dataContentCheck({ path: file.url, size: file.size }, 'img');
					if (isSafe) {
						const readUrl = await this.uploadFilePromise(file.url, file.type);
						console.log('插入图片数据', readUrl);
						this.insertImage(readUrl);
					}
				});
		 ...
		 * 上传成功 返回 文件链接
		 * */
		uploadFilePromise(path, type) {
			let fileName = 'images/' + uni.$u.guid(10) + '.' + type;

			this.ossToken.key = fileName;
			this.imgList.push(this.ossToken.host + '/' + fileName);
		...
		insertImage(src) {
			...
			that.editorCtx.insertImage({
				...
				//添加到图片 img标签上的类名为editor-img，设置前缀editor--才生效。
				//部分机型点击图片右边的光标时不灵敏，需将样式editor-img宽度调小 max-width:98%;
				//从而在图片右侧中留出部分位置供用户点击聚集。
				success(e) {
					//真机会自动插入一行空格
					console.log('插入后数据', e);
				}
