...
import { apiBusinessCommodityPage_Post,apiBusinessCommodityInfocommodityId_Get } from '@/API/商品好物news.js'; //列表
import { apiBusinessCommodityTagTagLabel_Get,apiBusinessCommodityTagInfoinfoTagId_Get } from '@/API/好物标签.js'; //标签
...
export default {
	...
	methods: {
		...
		getTagList(e) {
			apiBusinessCommodityTagTagLabel_Get({ cardId: e }).then(res => {
		...
		/* 获取标签详情数据,栏目风格、栏目名称 */
		getTagInfo(e) {
			apiBusinessCommodityTagInfoinfoTagId_Get({ infoTagId: e }).then(res => {
		...
		getNewsList(informationId) {
			...
			apiBusinessCommodityPage_Post(para)
				.then(res => {
		...
		toInfo(id) {
			console.log('传递数据', id);
			uni.navigateTo({
				url: './service_info?commodityId=' + id
			});
		}
	}
};
