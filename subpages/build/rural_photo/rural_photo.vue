	...
	<view class="page">
		...
		<u-navbar :placeholder="true" title=" " bgColor="#ffffff" leftIconColor="#fff" :titleStyle="{ color: '#333' }">
			<view class="u-nav-slot_left" slot="left">
				<view class="name"><text>乡村图集管理</text></view>
			</view>
...
import { apiBusinessAtlasPage_Post } from '@/API/图集.js'; //列表
...
export default {
	...
	methods: {
		goEdit(item) {
			uni.navigateTo({
				url: 'edit_photo?atlasId=' + item.atlasId + '&cardId=' + this.Pageinput.cardId
			});
		},

		goTag() {
			uni.navigateTo({
				url: 'photo_tag?cardId=' + this.Pageinput.cardId
			});
		},
		...
		mapList() {
			...
			apiBusinessAtlasPage_Post({
				...
				.then(res => {
					if (res.data.code == 200) {
						if (!this.checkData(res.data.data.records, '图集')) {
							return;
						}
		...
		pluson() {
			uni.navigateTo({
				url: '/subpages/card/card_photo/add_photo?cardId=' + this.Pageinput.cardId
			});
		}
	}
};
