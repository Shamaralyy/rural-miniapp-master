<template>
	<view class="content">
		<u-navbar title="图集标签管理" :autoBack="true" :placeholder="true" leftIcon="arrow-left" bgColor="#fff" leftIconColor="#333" :titleStyle="{'color':'#333'}">
		</u-navbar>
		<HM-dragSorts
		 :list="list" 
		 :isLongTouch="false" 
...
<script>
	import {apiBusinessAtlasTagAdd_Post,apiBusinessAtlasTagBatchUpdateSort_Post,apiBusinessAtlasTagList_Post,apiBusinessAtlasTagEdit_Post} from '@/API/图集标签.js';
	...
	export default {
		...
		methods: {
			...
			deleteEdit(){
				request({
				     url:'/api/business/atlasTag/remove/'+this.editInfo.infoTagId,
				     method:'DELETE',   
				     data: {
			...
			getTagList(){
				apiBusinessAtlasTagList_Post({ cardId: this.cardId })
					.then(res => {
			...
			saveEdit(){
				...
				apiBusinessAtlasTagEdit_Post(apiBusinessInformationTagEdit_Body).then(res => {
			...
			confirm(e){
				...
				console.log(sort)
				apiBusinessAtlasTagBatchUpdateSort_Post(sort)
				.then(res => {
