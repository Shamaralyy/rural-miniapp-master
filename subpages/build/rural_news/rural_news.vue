	...
	<view class="page">
		...
		<u-navbar :placeholder="true" title=" " bgColor="#ffffff" leftIconColor="#fff" :titleStyle="{ color: '#333' }">
			<view class="u-nav-slot_left" slot="left">
				<view class="name"><text>乡村资讯管理</text></view>
			</view>
...
import { apiBusinessInformationPage_Post } from '@/API/乡村名片资讯.js'; //列表
...
export default {
	...
	methods: {
		goEdit(item) {
			uni.navigateTo({
				url: 'edit_news?informationId=' + item.informationId + '&cardId=' + this.Pageinput.cardId
			});
		},

		goTag() {
			uni.navigateTo({
				url: 'news_tag?cardId=' + this.Pageinput.cardId
			});
		},
		...
		mapList() {
			...
			apiBusinessInformationPage_Post({
				...
				.then(res => {
					if (res.data.code == 200) {
						if (!this.checkData(res.data.data.records, '资讯')) {
							return;
						}
		...
		pluson() {
			uni.navigateTo({
				url: '/subpages/card/card_news/add_news?cardId=' + this.Pageinput.cardId
			});
		}
	}
};
