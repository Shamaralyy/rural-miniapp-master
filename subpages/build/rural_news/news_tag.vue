<template>
	<view class="content">
		<u-navbar title="资讯标签管理" :autoBack="true" :placeholder="true" leftIcon="arrow-left" bgColor="#fff" leftIconColor="#333" :titleStyle="{'color':'#333'}">
		</u-navbar>
		<HM-dragSorts
		 :list="list" 
		 :isLongTouch="false" 
...
<script>
	import {apiBusinessInformationTagEdit_Post,apiBusinessInformationTagList_Post,apiBusinessInformationTagRemoveinfoTagIds_Delete,apiBusinessInformationTagAdd_Post,apiBusinessInformationTagBatchUpdateSort_Post } from '@/API/资讯标签.js';
	...
	export default {
		...
		onShow() {
			...
			if (this.editShow && this.tempIconPara.name != undefined) {
				...
				console.log('√ 路径添加状态', this.tempIconPara);
			...
			if (this.editShow && this.tempStylePara.list_style.name != undefined) {
				...
				console.log('√ 路径添加状态', this.tempStylePara);
		...
		methods: {
			...
			deleteEdit(){
				request({
				     url:'/api/business/informationTag/remove/'+this.editInfo.infoTagId,
				     method:'DELETE',   
				     data: {
			...
			getTagList(){
				apiBusinessInformationTagList_Post({ cardId: this.cardId })
					.then(res => {
			...
			add(){
				...
				apiBusinessInformationTagAdd_Post({
			...
			saveEdit(){
				...
				apiBusinessInformationTagEdit_Post(apiBusinessInformationTagEdit_Body).then(res => {
			...
			confirm(e){
				...
				console.log(sort)
				apiBusinessInformationTagBatchUpdateSort_Post(sort)
				.then(res => {
