...
import {
	...
	apiBusinessInformationInfoinformationId_Get,
	apiBusinessInformationEdit_Post
...
export default {
	...
	methods: {
		...
		getNewsInfo() {
			var that = this;
			apiBusinessInformationInfoinformationId_Get({ informationId: this.informationId }).then(res => {
		...
		submit() {
			...
			//  return
			// }
			// let imgJson = JSON.stringify(this.fileList1);
			apiBusinessInformationEdit_Post({
				tagId: [this.checkboxValue1.id], //分类ID，数组结构
				informationId: this.informationId /** 坐标ID   integer required: */,
