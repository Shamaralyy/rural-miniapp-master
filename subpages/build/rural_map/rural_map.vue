<template>
	<view class="page">
		...
		<u-navbar :placeholder="true" title=" " bgColor="#ffffff" leftIconColor="#fff" :titleStyle="{ color: '#333' }">
			<view class="u-nav-slot_left" slot="left" @click="toTool">
				<view class="name"><text>乡村位置管理</text></view>
			</view>
			<!-- <view class="u-nav-slot_right" slot="right"
				:style="'right:' + menuwidth + 'px;bottom:' + menuwidth / 11 + 'px'">
				<view class="title"><text @click="page=1">地图预览</text></view>
			</view> -->
		</u-navbar>
		<!-- 导航  end-->
		...
		<view class="content_tabs">
			<u-tabs :list="tabs" @change="clicktabs" lineColor="#2aae67" :current="tabCurrent">
			...
			<view class="news_list" v-for="(item, index) in news_list" :key="index" @click="goEdit(item)">
				<view class="album">
					...
					<view class="content">
						...
						<u--text margin="10rpx 0 8px 0" :text="item.name"></u--text>
						<u--text :text="item.detail" size="12" margin="15rpx 0"></u--text>
						<view class="" style="width: 100%; padding-right: 20rpx;margin-bottom: 20rpx;">
							<map
								...
								:enable-scroll="false"
								:show-location="true"
								:markers="item.markers1"
								style="width: 300rpx!important; height: 150px;"
							></map>
						</view>
						<u-album :urls="item.images" keyName="url"></u-album>
...
export default {
	...
	methods: {
		goEdit(item) {
			console.log(item);
			uni.navigateTo({
				url: 'edit_map?coordinateId=' + item.coordinateId + '&cardId=' + this.Pageinput.cardId
			});
		},

		goTag() {
			uni.navigateTo({
				url: 'map_tag?cardId=' + this.Pageinput.cardId
			});
		},
		toTool() {
			console.log('点击', this.Pageinput.cardId);
			uni.navigateTo({
				url: '/subpages/build/rural_info/tools?cardId=' + this.Pageinput.cardId
			});
		},
		...
		dataProcessing(para) {
			...
			let temp = para.reverse();
			return temp.map((value, index) => {
				value.images = JSON.parse(value.images).map(vImg => vImg.imgUrl);
				value.markers1 = [
					{
		...
		mapList() {
			...
			apiBusinessCoordinatePage_Post({
				...
				.then(res => {
					if (res.data.code == 200) {
						if (!this.checkData(res.data.data.records, '坐标')) {
							return;
						}
						this.news_list = this.news_list.concat(this.dataProcessing(res.data.data.records));
		...
		pluson() {
			uni.navigateTo({
				url: '/subpages/card/card_map/add_map?cardId=' + this.Pageinput.cardId
			});
		}
	}
};
