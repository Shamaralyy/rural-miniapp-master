<template>
	<view class="content">
		<u-navbar title="坐标标签管理" :autoBack="true" :placeholder="true" leftIcon="arrow-left" bgColor="#fff" leftIconColor="#333" :titleStyle="{'color':'#333'}">
		</u-navbar>
		<HM-dragSorts
		 :list="list" 
		 :isLongTouch="false" 
		 ...
		 @onclick="onclick" >
			<template v-slot:rowContent="{row}">
				<view class="row">
					...
					<u-icon v-if="!row.status" name="eye-off"></u-icon>
				...
				<u--form labelPosition="left" :model="model1" labelWidth="100">
					...
					<u-form-item label="标签图标" prop="navInfo.url" borderBottom ref="item1">
						...
						<view @click="toChangePage(1)" v-if="editInfo.icon!=null" class="icon">
							<view class="">
								<u-icon :name="editInfo.icon" size="24"></u-icon>
							</view>
							<view class="">
								<text>更换</text>
							</view>
			...
			<view class="addForm">
				...
				<u--form labelPosition="left" :model="model1" labelWidth="100">
					<u-form-item label="标签名称" prop="tagInfo.name" borderBottom>
						<u--input v-model="model1.tagInfo.name" placeholder="请输入标签名称" border="none" @change="editTag()"></u--input>
					</u-form-item>
...
<script>
	import { apiBusinessCoordinateTag_Post,apiBusinessCoordinateTagPage_Post,apiBusinessCoordinateTagBatchUpdateSort_Post,apiBusinessCoordinateTagEdit_Post } from '@/API/坐标分类62.js';
	...
	export default {
		...
		methods: {
			toChangePage(e,id){
				if(e==1){
					uni.navigateTo({
						url:"/subpages/build/rural_diy/cubeIcon?type=1&icon=map"
					})
				}
				if(e==2){
			...
			deleteEdit(){
				request({
				     url:'/api/business/coordinateTag?tagId='+this.editInfo.coordinateTagId,
				     method:'DELETE',   
					 data: {
						 tagId:this.editInfo.coordinateTagId
					 }
				}).then(res => {
			...
			getTagList(){
				apiBusinessCoordinateTagPage_Post({ cardId: this.cardId })
					.then(res => {
			...
			saveEdit(){
				if(this.editInfo.status==true){
					this.editInfo.status = 1
				}
				if(this.editInfo.status==false){
					this.editInfo.status = 0
				}
				let apiBusinessCoordinateTagEdit_Body = {
				  coordinateTagId: this.editInfo.coordinateTagId,   /** ID   number required: */
				  introduce: this.editInfo.introduce,   /** 介绍   string required: */
				...
				apiBusinessCoordinateTagEdit_Post(apiBusinessCoordinateTagEdit_Body).then(res => {
			...
			confirm(e){
				...
				console.log(sort)
				apiBusinessCoordinateTagBatchUpdateSort_Post(sort)
				.then(res => {
					console.log('添加结果', res);
				})
