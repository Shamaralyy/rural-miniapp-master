	...
	<view>
		...
		<view class="content_form">
			<view class="form_list">
				<view class="title"><text>好物状态</text></view>
			...
			<view class="form_list" @click="pickerShow = true">
				<view class="title"><text>好物分类</text></view>
				<view class="info right">
        ...
        <view class="info right">
          <text @click="getAddress()">{{
			...
			<view class="form_list">
				<view class="title"><text>好物名称</text></view>
				<view class="info">
					<u--input
						placeholder="请输入好物名称"
						border="surround"
			...
			<view class="form_list up">
				<view class="title"><text>好物内容</text></view>
				<view class="info" style="width: 100%">
					<cu-editor
...
import { apiBusinessCommodityEdit_Post, apiBusinessCommodityInfocommodityId_Get } from '@/API/商品好物news.js'; //列表
...
export default {
	...
	methods: {
		...
		getNewsInfo() {
			var that = this;
			apiBusinessCommodityInfocommodityId_Get({ commodityId: this.commodityId }).then(res => {
		...
		submit() {
			...
			//  return
			// }
			apiBusinessCommodityEdit_Post({
				tagId: [this.checkboxValue1.id], //分类ID，数组结构
				commodityId: this.commodityId /** 坐标ID   integer required: */,
